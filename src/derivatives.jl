@doc raw"""
    Y = geodesic_derivative(M, p, X, t; γt = geodesic(M, p, X, t))
    geodesic_derivative!(M, Y, p, X, t; γt = geodesic(M, p, X, t))
...
"""
function geodesic_derivative(M, p, X, t; q = geodesic(M, p, X, t))
    return parallel_transport_to(M, p, X, q)
end
function geodesic_derivative!(M, Y, p, X, t; q = geodesic(M, p, X, t))
    parallel_transport_to!(M, Y, p, X, q)
    return Y
end

@doc raw"""
    Y = shortest_geodesic_derivative(M, p, X, t; γt = shortest_geodesic(M, p, q, t))
    shortest_geodesic_derivative!(M, Y, p, X, t; γt = shortest_geodesic(M, p, q, t))
...
* `γt = geodesic(M, p,X,t)` the point on the geodesic at ``t``.
  This way if the point was computed earlier it can be resued here.
"""
function shortest_geodesic_derivative(M, p, q, t; γt = shortest_geodesic(M, p, q, t))
